    ...
    while (regions.length > 3) {
      ...
      for (let i = 1; i < regions.length; i++) {
        if ((regions[i].end - regions[i].start) <
            (regions[smallest].end - regions[smallest].start)) {
          smallest = i;
        }
      }
      if (smallest > 0) {
        regions[smallest-1].end = regions[smallest].end;
      } else {
...
function renderPreview(lines, mode) {
  if (mode === "bedrock") return lines.join("<br>");
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < line.length; i++) {
      if (line[i] === S && i+1 < line.length) {
        const code = line[i+1];
        color = `#${MC_RGB[code][0].toString(16).padStart(2,"0")}${MC_RGB[code][1].toString(16).padStart(2,"0")}${MC_RGB[code][2].toString(16).padStart(2,"0")}`;
        i++;
      } else {
